...
| サイト | 状況 |
|---|---|
| Amazon | ⚠️ 専用パーサー(`worker/parsers/amazon.py`)は正常だが、**GitHub Actionsの共有クラウドIPがAmazonにボットと判定され、実際の運用では機能しない**（本物の商品ページではなくボット確認ページを返される。開発者の手元ネットワークからは正常に取得できることを確認済み）。自宅PCのセルフホストランナー移行で改善見込み |
| 楽天市場 | ✅ 対応（汎用判定: JSON-LD優先、キーワードフォールバック）。GitHub Actionsからの動作確認済み |
| Yahoo!ショッピング | ✅ GitHub Actionsからの到達性を確認済み。専用パーサー未実装のため汎用判定で動作 |
| SNKRDUNK | ✅ GitHub Actionsからの到達性を確認済み。専用パーサー未実装のため汎用判定で動作 |
| ZOZOTOWN | ❌ 非対応。開発者の手元ネットワークから403 Forbiddenで拒否される |
| タカラトミーモール（takaratomymall.jp） | ❌ 非対応。Akamaiのボット対策により全リクエストがタイムアウトし、応答が一切返らない（ヘッダー変更・60秒待機でも突破不可、開発者の手元ネットワークからも失敗するため自宅IPでも改善しない可能性が高い） |
...
**GitHub Actionsのクラウド共有IPは、大手ECサイトのボット対策に警戒されやすい。** 対策として、ユーザー自宅PCをGitHub Actionsのセルフホストランナーにして住宅IPから監視する方針を検討中（詳細は`作業.md`参照）。有料プロキシサービスは、実需要と収益が伴うまでは導入しない方針。
